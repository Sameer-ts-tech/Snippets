...
export default function EditSnippetScreen() {
  ...
  const { getSnippetById, updateSnippet } = useDB();
  ...
  useEffect(() => {
    if (id) {
      ...
      if (snippet) {
        ...
        setScreenshotUri(snippet.screenshot_uri || null);
      }
    }
  }, [id]);
  ...
  const handleUpdate = () => {
    ...
    try {
      updateSnippet(Number(id), {
        ...
        // Clear explanation cache if code content changes to keep AI descriptions fresh!
        explanation: null 
      });

      Alert.alert('Success', 'Snippet updated offline!', [
